...
{
//if your condition matches
if(count==size/2)
{
	s.pop();
	return;
}
//while checking the stack save the top element in backup for some time until the required element is deleted
int num=s.Top();
s.pop();
//recursively increment the count to reach the element you want to delete
solve(s,count+1,size);
//push the backup elements into the stack
s.push(num);
}
template<class T>
